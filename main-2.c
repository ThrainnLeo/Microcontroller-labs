      ...
      /*  //putstr();
        if (adc_flag_get(ADC0,ADC_FLAG_EOC)==SET) { // ...ADC done?
          if (adc_flag_get(ADC0,ADC_FLAG_EOIC)==SET) { //...ch3 or ch16?
            ...
            //LCD_ShowNum(20, 30, ((0x680-tmpr)/5)+25, 6, WHITE);
            //putch('a');
            ...
            //adcr = adcr>>8; //l88mem(4,adcr>>8);                  // ......move data
        ...
        if (usart_flag_get(USART0,USART_FLAG_RBNE)){ // USART0 RX?
          ...
          t = t + 10;
          LCD_ShowChar(t,50,usart_data_receive(USART0), OPAQUE, WHITE);
          //LCD_ShowNum())
        }
        ...
        if (t5expq()) {                     // Manage periodic tasks
            ...
            if ((key=keyscan())>=0) {       // ...Any key pressed?
              ...
              buff[i++] = lookUpTbl[key];
              if (i == 2){
                putch((buff[0]<<4) + buff[1]);
                i = 0;
              }
